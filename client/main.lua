...
function OpenStable(site)
    ...
    Core.Functions.TriggerCallback('bcc-stables:GetMyHorses', function(horseData)
        if horseData then
            SendNUIMessage({
                action = 'show',
...
RegisterNUICallback('BuyHorse', function(data, cb)
    ...
    if Stables[Site].trainerBuy and not IsTrainer then
        TriggerEvent('RSGCore:notifyRightTip',_U('trainerBuyHorse'), 'error', 4000)
        StableMenu()
        return
    end
    ...
    Core.Functions.TriggerCallback('bcc-stables:BuyHorse', function(canBuy)
        if canBuy then
            SetHorseName(data)
        else
            StableMenu()
    ...
    if GetOnscreenKeyboardResult() then
        ...
        if string.len(horseName) > 0 then
            ...
            if data.origin == 'updateHorse' then
                Core.Functions.TriggerCallback('bcc-stables:UpdateHorseName', function(nameSaved)
                    if nameSaved then
                        StableMenu()
                    end
                    IsNaming = false
            ...
            elseif data.origin == 'buyHorse' then
                data.captured = 0
                Core.Functions.TriggerCallback('bcc-stables:SaveNewHorse', function(horseSaved)
                    if horseSaved then
                        StableMenu()
                    end
                    IsNaming = false
            ...
            elseif data.origin == 'tameHorse' then
                ...
                Core.Functions.TriggerCallback('bcc-stables:SaveTamedHorse', function(horseSaved)
                    if horseSaved then
...
function GetSelectedHorse()
    Core.Functions.TriggerCallback('bcc-stables:GetHorseData', function(data)
        if data == false then
...
RegisterNUICallback('CloseStable', function(data, cb)
    ...
    if data.MenuAction == 'save' then
        Core.Functions.TriggerCallback('bcc-stables:BuyTack', function(result)
            if result then
                SaveComps()
            end
        end, data)
...
-- Function to get or create horse entity
function GetOrCreateHorseEntity()
    -- If we have a valid entity, use it
    if MyEntity and DoesEntityExist(MyEntity) and MyEntity ~= 0 then
        return MyEntity
    end
    
    -- Try to find nearby horse entity
    local playerPed = PlayerPedId()
    local coords = GetEntityCoords(playerPed)
    local horses = GetGamePool('CPed')
    
    for _, entity in pairs(horses) do
        if DoesEntityExist(entity) and IsEntityAPed(entity) then
            local model = GetEntityModel(entity)
            -- Check if it's a horse model (you might need to adjust this check)
            if IsThisModelAHorse(model) then
                local distance = #(coords - GetEntityCoords(entity))
                if distance < 10.0 then -- Within 10 units
                    print("Found nearby horse entity:", entity)
                    MyEntity = entity
                    return entity
                end
            end
    ...
    print("No valid horse entity found nearby")
    return nil
end

-- Updated SaveComps function
function SaveComps()
    print("=== SaveComps Debug Info ===")
    print("MyEntity value:", MyEntity)
    print("MyEntityID value:", MyEntityID)
    
    -- Try to get or find a valid horse entity
    local horseEntity = GetOrCreateHorseEntity()
    
    if not horseEntity then
        print("ERROR: No valid horse entity available")
        -- Try to spawn/get horse from database first
        TriggerEvent('bcc-stables:GetMyHorse', MyEntityID) -- Trigger event to get horse
        return
    end
    
    if not MyEntityID then
        print("ERROR: MyEntityID is nil - database ID not set")
        return
    end

    -- Update MyEntity with the valid entity
    MyEntity = horseEntity
    
    print("Using horse entity:", MyEntity)
    print("============================")

    local compData = {
        SaddlesUsing or 0,
        SaddleclothsUsing or 0,
        StirrupsUsing or 0,
        BagsUsing or 0,
        ManesUsing or 0,
        TailsUsing or 0,
        SaddleHornsUsing or 0,
        BedrollsUsing or 0,
        MasksUsing or 0,
        MustachesUsing or 0,
        HolstersUsing or 0,
        BridlesUsing or 0,
        HorseshoesUsing or 0
    }

    local compDataEncoded = json.encode(compData)

    if compDataEncoded and compDataEncoded ~= '[]' and compDataEncoded ~= 'null' then
        print("Attempting to save components for entity:", MyEntity, "DB ID:", MyEntityID)
        
        Core.Functions.TriggerCallback('bcc-stables:UpdateComponents', function(success)
            if success then
                print("Components saved successfully, applying to entity...")
                for i, component in ipairs(compData) do
                    if component and component ~= 0 then
                        print("Applying component", i, "value:", component)
                        SetComponent(MyEntity, component)
                    end
                end
                print("All components applied successfully")
            else
                print("Failed to save components to database")
            end
        end, compDataEncoded, MyEntityID)
    else
        print("No valid component data to save")
    end
end
...
function HorseDrinking()
    if not IsEntityInWater(MyHorse) then
        TriggerEvent('RSGCore:notifyRightTip',HorseName .. _U('needWater'), 4000)
        return
    end
...
AddEventHandler('bcc-stables:ManageHorseDeath', function()
    if not InWrithe then
        ...
        TriggerEvent('RSGCore:notifyRightTip',_U('horseWrithe'), 4000)
        ...
        if action then
            TriggerEvent('RSGCore:notifyRightTip',_U('horseDied'), 4000)
        end
...
function WhistleSpawn()
    if Config.whistleSpawn then
        ...
        TriggerEvent('RSGCore:notifyRightTip',_U('stableSpawn'), 4000)
    end
end
                ...
                if horseHash == horseModel then
                    ...
                    if Config.displayHorseBreed and not HorseBreed then
                        if horseCfg.breed == 'Other' then
                            TriggerEvent('RSGCore:notifyBottomRight', modelCfg.color, 4000)
                        else
                            TriggerEvent('RSGCore:notifyBottomRight', horseCfg.breed, 4000)
                        end
                        HorseBreed = true
                    end
                end
        ...
        for site, siteCfg in pairs(Trainers) do
            ...
            if (distance <= siteCfg.shop.distance) and (IsPedOnMount(playerPed)) and (mountNetId == tamedNetId) and (not IsNaming) then
                ...
                if Citizen.InvokeNative(0xE0F65F0640EF0617, SellTame) then  -- PromptHasHoldModeCompleted
                    Core.Functions.TriggerCallback('bcc-stables:CheckPlayerCooldown', function(onCooldown)
                        if onCooldown then
                            lib.notify({
                                title = _U('cooldown'),
                                description = _U('sellCooldown'),
                                type = 'error',
                                duration = 4000
                            })
                            HorseBreed = false
                        ...
                        if trainerOnly then
                            ...
                            if not IsTrainer then
                                lib.notify({
                                    title = _U('accessDenied'),
                                    description = _U('trainerSellHorse'),
                                    type = 'error',
                                    duration = 4000
                                })
                                HorseBreed = false
                        ...
                        if mount ~= 0 then
                            ...
                            lib.notify({
                                title = _U('horseSold'),
                                description = _U('tamedCooldown') .. Config.cooldown.sellTame .. _U('minutes'),
                                type = 'inform',
                                duration = 4000
                            })
                            DeleteEntity(mount)
                ...
                if Citizen.InvokeNative(0xE0F65F0640EF0617, KeepTame) then  -- PromptHasHoldModeCompleted
                    ...
                    if trainerOnly then
                        if not IsTrainer then
                            lib.notify({
                                title = _U('accessDenied'),
                                description = _U('trainerRegHorse'),
                                type = 'error',
                                duration = 4000
                            })
                            HorseBreed = false
                    ...
                    Core.Functions.TriggerCallback('bcc-stables:RegisterHorse', function(canKeep)
                        if canKeep then
...
AddEventHandler('bcc-stables:ReviveHorse', function()
    ...
    if not hasItem then
        TriggerEvent('RSGCore:notifyRightTip',_U('noReviver'), 4000)
        return
    end
...
function OpenInventory(horsePedId, horseId, isLooting)
    ...
    if not isLooting and Config.useSaddlebags and not hasSaddlebags then
        TriggerEvent('RSGCore:notifyRightTip',_U('noSaddlebags'), 4000)
        return
    end
...
function ReturnHorse()
    ...
    if not MyHorse or MyHorse == 0 then
        TriggerEvent('RSGCore:notifyRightTip', _U('noHorse'), 4000)
        return
    end
    ...
    DeleteEntity(MyHorse)
    MyHorse = 0
    TriggerEvent('RSGCore:notifyRightTip',_U('horseReturned'), 4000)
end
...
function SaveXp(xpSource)
    ...
    if Config.showXpMessage then
        TriggerEvent('RSGCore:notifyRightTip','+ ' .. horseXp .. ' XP', 2000)
    end
...
RegisterNetEvent('bcc-stables:BrushHorse', function()
    if not MyHorse or MyHorse == 0 then
        return TriggerEvent('RSGCore:notifyRightTip',_U('noHorse'), 4000)
    end
    ...
    if distance > 3.5 then
        return TriggerEvent('RSGCore:notifyRightTip',_U('tooFar'), 4000)
    end
...
RegisterNetEvent('bcc-stables:FeedHorse', function(item)
    ...
    if distance > 3.5 then
        TriggerEvent('RSGCore:notifyRightTip',_U('tooFar'), 4000)
        return
    end
...
RegisterNetEvent('bcc-stables:FlamingHooves', function()
    ...
    TriggerEvent('RSGCore:notifyRightTip',_U('flameHoovesActivated'), 4000)
    Activated = true
    ...
    Citizen.SetTimeout(duration, function()
        if DoesEntityExist(MyHorse) then
            ...
            TriggerEvent('RSGCore:notifyRightTip',_U('flameHoovesDeactivated'), 4000)
            Activated = false
        end
    end)
end)
...
RegisterNUICallback('sellHorse', function(data, cb)
    ...
    Core.Functions.TriggerCallback('bcc-stables:SellMyHorse', function(horseSold)
        if horseSold then
            StableMenu()
        end
    end, data)
...
function CheckPlayerJob(trainer, site, cb)
    Core.Functions.TriggerCallback('bcc-stables:CheckJob', function(result)
        IsTrainer = false
        HasJob = false
        ...
        if result then
            ...
            if not trainer and not result[1] and Stables[site] and Stables[site].shop and Stables[site].shop.jobsEnabled then
